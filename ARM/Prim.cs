    ...
    public static class Prim
    {
        public static List<Enlace> Ejecutar(Grafo g){
            ...
            {                                                    // Si el grafo tiene nodos
                ...
                List<Enlace> enlaces = g.GetEnlaces(n);          // Obtengo todos los enlaces que 
                                                                 //     contenga a n cuyo otro nodo 
                                                                 //     no este visitado
                while (!g.TodosVisitados())                      // Mientras no esten todos los nodos visitados
                {
                    ...
                    n.visitado = true;                           // Marco el nodo como visitado
                    enlaces.AddRange(g.GetEnlaces(n));           // Agrego a la lista de enlaces a visitar todos 
                                                                 //    los enlaces del nuevo nodo
                    enlaces = RemoverEnlaces(enlaces);           // Elimino todos los enlaces con los 2 nodos 
                                                                 //    visitados de la lista
                    resultado.Add(enlaceMenor);                  // Agrego el enlace menor a la lista 
                                                                 //     de resultado
                }
            }
